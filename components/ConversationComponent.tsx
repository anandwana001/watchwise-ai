...
export default function ConversationComponent({
  ...
  useEffect(() => {
    ...
    const orderedMessages = [...messageList, ...(currentInProgressMessage ? [currentInProgressMessage] : [])];
    const transcriptText = orderedMessages
      .map((message) => message.text ?? '')
      .filter(Boolean)
      .join(' ');
    const latestUserText =
      [...orderedMessages].reverse().find((message) => String(message.uid) !== agentUID)?.text ?? '';
  ...
  }, [agentUID, currentInProgressMessage, messageList, onTranscriptUpdate]);
